...
def linear_evaluation(encoder, device, epochs=20, batch_size=256):
    """
    Standard linear eval on frozen encoder using full CIFAR100 train set.
    ...
    model = LinearEvalModel(encoder, num_classes=100).to(device)
...
-
# Build SAS / Random indices -

def build_sas_and_random_indices(device, fractions) -> (Dict[float, List[int]], Dict[float, List[int]]):
    """
    Uses the sas library to compute SAS and random subset indices for CIFAR100.
    Returns:
    ...
    cifar_for_sas = torchvision.datasets.CIFAR100(
        root="./data",
        train=True,
        download=True,
    ...
    # Load proxy model from repo
    proxy_path = "proxy-cifar100-resnet10-399-net.pt"
...
def main():
    ...
    # Training settings
    pretrain_epochs = 10   # SimCLR epochs
    linear_epochs = 20     # linear eval epochs
    batch_size = 256

    # 1. Full data baseline
    print("\n========== FULL DATA ==========")
    cifar_full_raw = torchvision.datasets.CIFAR100(
        root="./data",
        train=True,
        download=True,
    ...
    # Base raw CIFAR100 (PIL) to build actual subsets for SimCLR
    base_raw = torchvision.datasets.CIFAR100(
        root="./data",
        train=True,
        download=True,
        transform=None,    # PIL
    ...
    plt.title("CIFAR100 SimCLR + SAS vs Random")
    plt.legend()
